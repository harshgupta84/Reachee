  ...
  try {
    ...
    if (error) {
      return NextResponse.redirect(
        new URL(`/dashboard?tab=profile&error=Instagram authentication failed: ${error}`, request.url)
      );
    }
    ...
    try {
      ...
      const accessToken = await SocialMediaValidator.exchangeInstagramCode(code);
      
      // Validate the account and get profile info
      const validationResult = await SocialMediaValidator.validateInstagram('', accessToken);
      ...
      return NextResponse.redirect(
        new URL(`/dashboard?tab=profile&instagram_connected=true&account_data=${accountData}`, request.url)
      );

    } catch (tokenError) {
      return NextResponse.redirect(
        new URL(`/dashboard?tab=profile&error=Failed to process Instagram authentication: ${tokenError}`, request.url)
      );
    }
    ...
    return NextResponse.redirect(
      new URL('/dashboard?tab=profile&error=Instagram authentication failed', request.url)
    );
  }
} 